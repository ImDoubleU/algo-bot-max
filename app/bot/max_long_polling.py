    ...
    def first_entry_response(self, user_id: int | None) -> BotResponse:
        ...
        return BotResponse(
            (
                ...
                "Родителю понадобится персональная ссылка или ID из письма школы, "
                "ребенку — QR-код из кабинета преподавателя или родителя."
            ),
            role_selection_keyboard(),
        )
    ...
    def handle_student_invitation_response(
        ...
        *,
        ...
        if not token or self.backend_client is None:
            return BotResponse(
                "Ссылка ученика недоступна. Попросите преподавателя или родителя "
                "открыть новый QR-код.",
                self.main_menu_attachments(user_id),
            )
        ...
        if role == "student":
            ...
            return BotResponse(
                (
                    ...
                    "Попросите преподавателя открыть QR-код ученика в рабочем кабинете. "
                    "Если родитель уже подключен, QR-код также доступен в его кабинете."
                ),
                onboarding_cancelled_keyboard(),
            )
