...
def add_cover(guide: Any, prs: Presentation) -> None:
    ...
    guide.add_text(slide, "50+ актуальных экранов", 0.86, 5.75, 3.26, 0.24, size=13, bold=True)
    guide.add_text(slide, "Редакция 6 · интерфейс 0.78.1 · август 2026", 0.66, 6.70, 4.10, 0.24, size=10.2, fill="CBB9E8")
...
def _management_slides() -> tuple[VisualSlide, ...]:
    return (
        ...
        VisualSlide("Сотрудники", "Активные и отозванные роли", "Управление -> Сотрудники", "staff-list-desktop.png", (
            Hotspot(0.28, 0.35, "Выберите состояние", "Активные и отозванные роли разделены."),
            Hotspot(0.55, 0.47, "Найдите сотрудника", "Поиск работает по имени, MAX ID и роли."),
            Hotspot(0.93, 0.63, "Откройте действия", "Меню позволяет настроить или отозвать роль."),
        ), note="Директор может отзывать роли администраторов своего города."),
        ...
        VisualSlide("Товары", "Массовый импорт товаров", "Товары и остатки -> Массовый импорт", "product-import-desktop.png", (
            Hotspot(0.30, 0.40, "Скачайте шаблон", "Не переименовывайте обязательные колонки."),
            Hotspot(0.55, 0.55, "Выберите файл", "Поддерживается подготовленный XLSX или CSV."),
            Hotspot(0.82, 0.68, "Проверьте результат", "Ошибочные строки исправляются до повторной загрузки."),
        ), note="Шаблон содержит пример и правила для физических и цифровых товаров."),
...
def build_visual_presentation(guide: Any) -> Presentation:
    ...
    for index, spec in enumerate(management_specs):
        ...
        if index == 3:
            guide.add_steps_slide(
                prs, kicker="Сотрудники", title="Отзыв роли и уведомления",
                steps=[("Найдите сотрудника", "Используйте поиск по имени, MAX ID или роли."), ("Откройте меню", "Кнопка действий находится справа в строке."), ("Настройте уведомления", "Измените нужные категории сообщений."), ("Отзовите роль", "Директор может отозвать администратора своего города."), ("Проверьте", "Связь перейдет во вкладку «Отозванные».")],
                side_title="Что сохраняется", side_items=["История действий.", "Данные учеников.", "Заказы и остатки.", "Возможность выдать новую роль."], callout_title="Отзывайте роль, не аккаунт", callout_text="Это прекращает полномочия без удаления связанных данных.", callout_kind="yellow",
            )
        if index == 9:
            guide.add_steps_slide(
