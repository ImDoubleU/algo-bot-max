...
5. Проверяет сформированный текст, копирует его или сохраняет как черновик.
...
```text
https://max.ru/<MAX_BOT_USERNAME>?start=shop_<CRM_CONTACT_ID>
```

Вместо `<CRM_CONTACT_ID>` CRM подставляет Contact ID родителя. После перехода
бот создает связи со всеми найденными детьми, показывает приветствие и кнопку
перехода в магазин. Тот же шаблон можно получить скрытой служебной командой
`/link <CRM_CONTACT_ID>`.
