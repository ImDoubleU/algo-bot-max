...
паузы в разделе «Управление» -> «Ученики». Дни внутри паузы не расходуют срок доступа: например,
...
только колонки `ID ученика`, `ID сделки amoCRM`, `Фамилия ребенка`, `Имя ребенка`, `Группа`, `Курс`, `Площадка`,
`Преподаватель`, `ID контакта` и `Имя контакта`; остальные листы и колонки игнорируются.
...
В разделе `Управление -> Ученики` администратор, директор или суперадминистратор
может добавить ученика вручную, изменить его состояние и задать точный итоговый
баланс. Изменение баланса сохраняется отдельной операцией, а добавление и смена
состояния остаются в истории ученика.
