# MAX-бот и приложение Algo MAX

Рабочий проект MAX-бота, backend API и приложения для астрокоинов, магазина наград, заказов, складов и управления филиалами.

Production-развертывание на российском VPS: [DEPLOYMENT_RU.md](DEPLOYMENT_RU.md).

Материалы по продукту: [полное руководство, презентация и видео](docs/PRODUCT_MATERIALS_RU.md).

Редактируемое руководство по всем ролям: [Algo_MAX_Руководство_по_ролям.pptx](docs/Algo_MAX_Руководство_по_ролям.pptx).

Краткая презентация продукта: [Algo_MAX_Презентация_продукта.pptx](docs/Algo_MAX_Презентация_продукта.pptx).
...
сводка файла без записи и распределение по городам, затем отдельная кнопка подтверждает
импорт. Строки автоматически попадают в существующие города того же партнера, доступные
текущему сотруднику. Неизвестный город или отсутствие прав останавливают весь импорт до записи.
...
`Дата рождения`, `Город`, `Группа`, `Курс`, `Площадка`, `Преподаватель`, `ID контакта` и
`Имя контакта`; остальные колонки игнорируются. Можно загружать стандартный лист «Шаблон»
или исходный лист «Сделки» из CRM. В обоих случаях колонка `Город` обязательна.
...
Каждый день сервер начисляет активным именинникам сумму из настройки
`Начисления -> Настроить причины -> С днем рождения` (по умолчанию `50 AC`).
Операция выполняется один раз в год на ученика; повторный запуск не меняет баланс
второй раз. Для родившихся 29 февраля подарок в невисокосный год начисляется 28 февраля.
...
### Ежедневная обработка банка

Банковские проценты, капитализация, завершение срока и страховочное закрытие
вкладов неактивных учеников обрабатываются одной идемпотентной командой:

```powershell
max_bot_venv\Scripts\python.exe -m app.cli.process_bank_accounts
max_bot_venv\Scripts\python.exe -m app.cli.process_bank_accounts --date 2026-09-06
```

Без `--date` используется текущая дата в `APP_TIMEZONE`. Команда автоматически
догоняет пропущенные процентные дни; повторный запуск за ту же дату не создает
повторных начислений. В серверный комплект входят `algo-max-bank.service` и
`algo-max-bank.timer`: timer запускает обработку ежедневно после полуночи и имеет
`Persistent=true`. Отдельные переменные окружения для банка не требуются.
