...
- куратор: ученики, заказы, начисления, расписание, обратная связь и рассылки по филиалу;
...
Автоматическую подготовку ОС преподавателю запустить отдельным процессом:
...
Worker готовит ОС и отправляет результат преподавателю. Родительская доставка ОС отключена.
...
`MAX_ORDER_NOTIFICATIONS_ENABLED=true` включает сообщения MAX покупателям по заказам и служебные уведомления сотрудникам по заказам, журналу, рассылкам и малым остаткам. Получателей служебных уведомлений директор или суперадминистратор настраивает в разделе сотрудников.
...
5. Проверяет сформированный текст и отправляет его связанным родителям кнопкой.

Ручные черновики сохраняются отдельно и доступны только через раздел MAX-бота. Повторная
отправка уже доставленного черновика не дублирует сообщения.
В mini-app настраиваются расписание, курсы и автоматическая подготовка ОС после занятия.
Права и список групп проверяются
...
Для фактической отправки 12 сообщений с пометкой «ТЕСТ» добавьте `--confirm-send`.
Команда проверяет ручную рассылку с изображением, товар с изображением, низкий остаток, все статусы заказа, ОС
...
Во вкладке «Расписание» преподаватель выбирает свою группу и дату урока,
отмечает присутствовавших учеников и сохраняет журнал. Куратор и управляющие
роли могут работать со всеми группами текущего tenant.
